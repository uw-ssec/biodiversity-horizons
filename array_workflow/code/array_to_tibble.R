...
array_to_tibble <- function(array.data, 
  ...
  if(is.null(temporal.resolution)) stop("Provide the temporal resolution of the climate data: 'yearly' or 'monthly'")
  if(!temporal.resolution %in% c("yearly", "monthly")) stop("Temporal resolution should be 'yearly' or 'monthly'")
  ...
  if("member" %in% names(dim(temp_array))){
    members <- 1L:dim(temp_array)[["member"]]
  } else {
    members <- 1L
  }
